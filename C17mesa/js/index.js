...
function renderizarDatosUsuario(datos) {
    ...
    tarjeta.innerHTML += `<img src="${datos.picture.large}" alt="">
                             <h2>${datos.name.first}</h2>
                             <h2>${datos.name.last}</h2>
                            <p>${datos.email}</p>`;
...
let botonRandom = document.querySelector("button#random");
botonRandom.addEventListener("onclick", function(event) {
    if(event){
