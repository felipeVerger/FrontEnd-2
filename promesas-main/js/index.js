...
consultandoBaseDeDatos
    .then((response) => {
        ...
        let renderizado = renderizarDatosUsuario(resolve);
    })
    .catch((err) => {
        console.log(err);
    });
...
function renderizarDatosUsuario(datos) {
    ...
    datos.results.forEach(function(dato) {
        contenedor.innerHTML += `<img src="${dato.results.picture}" alt="">
                                <h3>${dato.results.name}</h3>
                                <p>${dato.results.email}</p>`
    });
